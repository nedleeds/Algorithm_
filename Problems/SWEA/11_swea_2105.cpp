...
void checkIn(Node now, int value) {
	visit[now.r][now.c] = value;
	DAT[map[now.r][now.c]] = value;
	maxValue = value;
}

bool isStart(Node now, Node start) {
	if (now.r == start.r && now.c == start.c)
		return true;
	else
		return false;
}

bool isEscape(Node now, Node start, int dir, int eatCnt) {
	if (dir < 3) {
		dir += 1;
	}

	Node next = { now.r + dR[dir], now.c + dC[dir] };
	if (isStart(next, start) && eatCnt >= 3) {
		return true;
	}
	return false;
}

void dfs(Node now, Node start, int dir, int eatCnt) {
	if (isEscape(now, start, dir, eatCnt)) {
		dessert = max(dessert, maxValue);
		return;
	}
	...
	for (int i = dir; i < 4; i++) {
		...
		if (visit[next.r][next.c] != 0)
			continue;
		if (DAT[map[next.r][next.c]] != 0)
			continue;


		checkIn(next, visit[now.r][now.c] + 1);
		dfs(next, start, i, eatCnt + 1);
		checkIn(next, 0);
	}
